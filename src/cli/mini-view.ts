...
import type { ProjectStatus, Todo, Project, TadaStore } from "../core/types.js";
...
function renderProjectSummary(projects: Project[], store: TadaStore): string {
	...
	const totalTodos = projects.reduce(
		(sum, p) => sum + getProjectTodos(store, p.id).length,
		0,
	);
	const parts = [
...
export function renderMiniProjectView(
	store: TadaStore,
	...
	if (filter === "all") {
		...
		projects = store.projects.filter((p) => p.status === filter);
	} else {
		projects = store.projects.filter((p) => p.status === "active");
	}

	// Sort by position within each status bucket (active first, then on_hold, then completed)
