...
describe("getToday", () => {
	...
	it("includes overdue items (past scheduledDate)", () => {
		const store = emptyStore();
		store.todos = [makeTodo({ id: "overdue1", scheduledDate: pastDate(3) })];
	...
	it("includes overdue items (past deadline)", () => {
		const store = emptyStore();
		store.todos = [makeTodo({ id: "overdue1", deadline: pastDate(2) })];
	...
	it("includes undated todos (no scheduled date or deadline)", () => {
		const store = emptyStore();
		store.todos = [makeTodo({ id: "nodate01" })];
	...
	it("excludes future-scheduled todos", () => {
		const store = emptyStore();
		store.todos = [makeTodo({ id: "future01", scheduledDate: futureDate(5) })];
	...
	it("excludes completed undated todos", () => {
		const store = emptyStore();
		store.todos = [makeTodo({ id: "done0001", status: "completed" })];
...
describe("getUpcoming", () => {
	...
	it("excludes todos with no scheduledDate", () => {
		const store = emptyStore();
		store.todos = [makeTodo({ id: "nodate01", deadline: futureDate(5) })];
...
describe("getByTag", () => {
	...
	it("returns empty array when no todos have the tag", () => {
		const store = emptyStore();
		store.todos = [makeTodo({ id: "notag001", tags: [] })];
...
describe("search", () => {
	...
	it("is case-insensitive", () => {
		const store = emptyStore();
		store.todos = [makeTodo({ id: "todo0001", title: "Buy GROCERIES" })];
	...
	it("returns both todos and projects that match", () => {
		const store = emptyStore();
		store.todos = [makeTodo({ id: "todo0001", title: "Design mockup" })];
		store.projects = [makeProject({ id: "proj0001", title: "Design system" })];
...
describe("getAllTodos", () => {
	it("returns open top-level todos regardless of project or schedule", () => {
		const store = emptyStore();
		store.todos = [
			makeTodo({ id: "inbox001", title: "Inbox A", projectId: null }),
			makeTodo({
				id: "proj0001",
				title: "In project",
				projectId: "projXXXX",
	...
	it("excludes completed and cancelled todos", () => {
		...
		store.todos = [
			...
			makeTodo({ id: "xxxx0001", status: "cancelled" }),
		];

		const all = getAllTodos(store);
		expect(all).toHaveLength(1);
		expect(all[0].id).toBe("open0001");
	});

	it("excludes subtasks", () => {
		const store = emptyStore();
		store.todos = [
			makeTodo({ id: "parent01" }),
			makeTodo({ id: "sub00001", parentId: "parent01" }),
		];

		const all = getAllTodos(store);
		expect(all).toHaveLength(1);
		expect(all[0].id).toBe("parent01");
	});
});
